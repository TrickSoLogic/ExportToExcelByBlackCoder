...
namespace ExportExcel_in_ASPNetCore.Controllers
{
    public class HomeController : Controller
    {
        ...
        {
            List<BlackCoder> blackcoder = _context.blackcoder.ToList();
            return View(blackcoder);
        }
        [HttpPost]
        public IActionResult ExportExcel()
        {
            var blackcoder = _context.blackcoder.ToList();
            using (XLWorkbook wb = new XLWorkbook())
            {
                wb.Worksheets.Add(Common.ToDataTable(blackcoder.ToList()));
